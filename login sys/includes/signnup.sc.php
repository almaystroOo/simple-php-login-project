...
if (isset($_POST['sisub'])) {
     ...
     if ($password !== $passwordReapet) {
          ...
          if (!mysqli_stmt_prepare($stmt, $sql)) {
               header("location: userstb.php?error=sqlerror&" . $username);
               exit();
          } else {
               $result = "";
               //$hash = md5($password);
               ...
               if ($row['idUsers'] > 0) {
                    ...
                    $stmt = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($stmt, $sql)) {
                         header("location: ../signup.php?error=sqlerror&" . $username);
                         exit();
                    } else {
                         //$hash = md5($password);
                         $hashedpwd = password_hash($password, PASSWORD_DEFAULT);
                         ...
                         mysqli_stmt_execute($stmt);
                         header("location: ../signup.php?signup=success&user=" . $username . "&resultcheck=" . $resultCheck . "get=" . $get);
                         exit();
                    }
               }
          }
